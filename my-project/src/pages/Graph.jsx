...
function buildConfig(graph, chartData) {
  ...
  const isArcChart   = key === "Pie" || key === "Doughnut";
  ...
  const datasets = chartData.datasets.map((ds, i) => {
    ...
    if (isArcChart) {
      return {
        ...ds,
        backgroundColor: COLORS.map((col) => col.bg),
 ...
 return {
    ...
    options: {
      ...
      scales: scalesConfig,
      ...(isArcChart && { cutout: key === "Doughnut" ? "55%" : "0%" }),
    },
  };
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pr-6">
        ...
        >
          <ChartIcon />
          {graph.chartType?.trim()}
        </span>
        {isLoading
          ? <span className="w-3.5 h-3.5 rounded-full border-2 border-slate-700 border-t-sky-400 animate-spin" />
          : <span className="w-2 h-2 rounded-full" style={{ background: isSelected ? tm.color : "#1e293b" }} />}
      </div>

      <div className="space-y-1.5">
        {[
          { label: "X", fields: graph.xAxis || [], col: "#38bdf8" },
          { label: "Y", fields: graph.yAxis || [], col: "#fbbf24" },
        ].map(({ label, fields, col }) => (
  ...
  return (
    <>
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
        ...
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-lg flex items-center justify-center border border-slate-700 text-slate-500
                     hover:text-slate-300 hover:border-slate-500 hover:bg-slate-800 transition-all"
        >
          <I.X />
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 px-6 py-3 border-b border-slate-800/70 shrink-0">
        {[
          ...
          { label: "Y Axis", items: graph.yAxis || [], col: "#fbbf24" },
        ].map(({ label, items, col }) => (
          <div key={label} className="flex flex-col gap-1">
            ...
            <div className="flex flex-wrap gap-1">
              {items.length > 0
                ...
                : <Tag style={{ color: "#475569", background: "rgba(30,41,59,0.6)", borderColor: "rgba(51,65,85,0.5)" }}>—</Tag>}
            </div>
          </div>
        ))}
...
export default function Graph() {
  ...
  const chartTypes = [
    "All",
    ...new Set(
      (graphs || []).filter(Boolean).map((g) => normalizeChartKey(g.chartType || "")).filter(Boolean)
    ),
  ];
  ...
  const filtered =
    filter === "All"
      ...
      : (graphs || []).filter(Boolean).filter((g) => normalizeChartKey(g.chartType || "") === filter);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto gd-scroll">
            ...
            {chartTypes.map((t) => {
              const count =
                t === "All"
                  ? (graphs?.length ?? 0)
                  : (graphs || []).filter((g) => normalizeChartKey(g.chartType || "") === t).length;
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: active ? `${tm.color}14` : "transparent",
                    border:     active ? `1px solid ${tm.color}33` : "1px solid transparent",
                  }}
                >
                  <NavIcon />
                  ...
                  <span
                    ...
                    style={{
                      background: active ? `${tm.color}22` : "rgba(30,41,59,0.8)",
                      color:      active ? tm.color : "#334155",
                    }}
                  >
                    {count}
                  </span>
          ...
          <div
            className="px-5 py-3 text-[10px] text-slate-700"
        ...
        >
          ...
          >
            <div>
              <h1
                className="text-[17px] font-bold text-slate-100"
                style={{ fontFamily: "sans-serif", letterSpacing: "-0.4px" }}
              >
                Graph Library
              </h1>
          ...
          <div className="flex-1 overflow-y-auto gd-scroll p-8">
            {fetching && (!graphs || graphs.length === 0) ? (
              <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
                {[...Array(6)].map((_, i) => (
                  <div
                    ...
                    style={{ background: "rgba(15,23,42,0.6)", animationDelay: `${i * 60}ms` }}
                  />
                ))}
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 gap-3 text-slate-700">
                <I.Empty />
                <p className="text-sm font-semibold text-slate-600" style={{ fontFamily: "sans-serif" }}>
                  No charts found
                </p>
